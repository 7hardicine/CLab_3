...
void InputPlane(struct Planes** plane, int* struct_size)
{
	...
	*plane = (struct Planes*)realloc(plane, (*struct_size + old_struct_size) * sizeof(struct Planes));
	for (int i = 0; i < *struct_size; i++)
	{
		...
		printf_s("Введите название самолёта: ");
		scanf_s(" %s", (*plane + old_struct_size + i)->mark, (unsigned)countof((*plane + old_struct_size + i)->mark));
...
{
	puts("|===================================================================|");
	printf_s("|%4s|%s|%s|%s|%s|%s|", "#", "Борт.номер", "Вместимость", "Грузоподъёмность", "Название", "Год выпуска");
	for (int i = 0; i < struct_size; i++)
	{
		puts("Я");
	}
